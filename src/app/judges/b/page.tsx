...
export default function JudgeBPage() {
  ...
  const handleGenerate = async () => {
    ...
    const res = await fetch('/api/judges/schedule/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ category: 'b', n: parseInt(genN) || 2 }) })
                ...
                return (
                  <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-4">
                    ...
                    <div className="text-sm font-medium text-gray-700">
                      {teamName(activeMatch.team1_id)} <span className="text-gray-400 font-normal">vs</span> {teamName(activeMatch.team2_id!)}
                    </div>
                    {r ? (
                      <div className="space-y-3">
                        <div className="bg-gray-50 rounded-lg p-3">
                          <div className="text-[10px] font-bold text-gray-400 uppercase mb-1">Winner</div>
                          <div className="font-black text-gray-900">{winnerLabel(r)}</div>
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          <div className="bg-gray-50 rounded-lg p-3">
                            <div className="text-[10px] font-bold text-gray-400 uppercase mb-1">Rounds T1</div>
                            <div className="font-mono font-bold text-gray-900 text-xl">{r.rounds1}</div>
                          </div>
                          <div className="bg-gray-50 rounded-lg p-3">
                            <div className="text-[10px] font-bold text-gray-400 uppercase mb-1">Rounds T2</div>
                            <div className="font-mono font-bold text-gray-900 text-xl">{r.rounds2}</div>
                          </div>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-3">
                          <div className="text-[10px] font-bold text-gray-400 uppercase mb-1">Starting Position</div>
                          <div className="font-bold text-gray-900 capitalize">{r.starting_position}</div>
                        </div>
                        {r.notes && (
