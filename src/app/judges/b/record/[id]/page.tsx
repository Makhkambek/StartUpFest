  ...
  useEffect(() => {
    Promise.all([
      ...
      const ex = Array.isArray(results)
        ? (results.find((r: MatchB) => r.scheduled_match_id === id) ??
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-zinc-950">
      <header className="bg-white dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-700 h-14 flex items-center px-3 sm:px-6 gap-4 sticky top-0 z-10">
        <button onClick={() => router.push('/judges/b')} className="text-sm text-gray-400 dark:text-zinc-400 hover:text-gray-700 dark:hover:text-zinc-200">← Back</button>
      ...
      <div className="p-3 sm:p-6 max-w-lg mx-auto space-y-4">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-800 shadow-sm p-5 space-y-5">
          {/* Starting Position */}
          <div>
            <label className="text-xs font-bold text-gray-400 dark:text-zinc-400 uppercase tracking-wide block mb-2">Starting Position</label>
            <div className="flex gap-2">
              {(['face', 'side', 'back'] as const).map(pos => (
                <button key={pos} onClick={() => setStartingPosition(pos)}
                  className={`flex-1 py-3 rounded-lg text-sm font-bold border transition-colors ${startingPosition === pos ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 dark:border-zinc-700 text-gray-500 dark:text-zinc-400 hover:bg-gray-50 dark:hover:bg-zinc-800'}`}>
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setWinner(0)}
                className={`px-4 py-3 rounded-lg text-sm font-bold border transition-colors ${winner === 0 ? 'bg-gray-500 text-white border-gray-500' : 'border-gray-200 dark:border-zinc-700 text-gray-500 dark:text-zinc-400 hover:bg-gray-50 dark:hover:bg-zinc-800'}`}>
                Draw
              </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-gray-400 dark:text-zinc-400 uppercase tracking-wide block mb-2">Rounds — {teamName(match.team1_id)}</label>
              <input type="number" min="0" step="1" value={rounds1} onChange={e => setRounds1(e.target.value)}
                placeholder="0"
                className="w-full border border-gray-200 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 dark:placeholder-zinc-500 rounded-lg px-4 py-3 text-base text-xl font-mono focus:outline-none focus:ring-2 focus:ring-blue-300" />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-400 dark:text-zinc-400 uppercase tracking-wide block mb-2">Rounds — {match.team2_id ? teamName(match.team2_id) : 'Team 2'}</label>
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3}
              ...
              className="w-full border border-gray-200 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 dark:placeholder-zinc-500 rounded-lg px-4 py-3 text-base text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 resize-none" />
          </div>
            ...
            <div className="bg-green-50 dark:bg-green-950/20 border-2 border-green-200 dark:border-green-900 rounded-xl p-4 space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={undo} disabled={!savedRecordId}
                  className="flex-1 bg-white border-2 border-red-300 dark:border-red-800 text-red-700 dark:text-red-400 py-3 rounded-lg text-sm font-bold hover:bg-red-50 dark:hover:bg-red-950/30 dark:bg-red-950/30 disabled:opacity-40">
                  ↩ Undo
                </button>
