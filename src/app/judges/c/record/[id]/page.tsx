...
export default function RecordCPage() {
  ...
  const saveNoShow = async (noShowTeam: 1 | 2) => {
    ...
    setSaving(true); setSaveMsg('')
    if (existingRecordId) {
      await fetch('/api/judges/c/fights', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: existingRecordId }) })
      setExistingRecordId(null)
    }
    const res = await fetch('/api/judges/c/fights', {
      method: 'POST',
