  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <header className="bg-white border-b border-gray-200 h-14 flex items-center px-3 sm:px-6 gap-4 sticky top-0 z-10">
        <button onClick={() => router.push('/judges/c')} className="text-sm text-gray-400 hover:text-gray-700">← Back</button>
      ...
      <div className="p-3 sm:p-6 max-w-lg mx-auto space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-5">
          {/* Method */}
          <div>
            <label className="text-xs font-bold text-gray-400 uppercase tracking-wide block mb-2">Decision Method</label>
            <div className="flex gap-2">
              {([
                ...
                <button key={m.value} onClick={() => setMethod(m.value)}
                  className={`flex-1 py-3 rounded-lg text-sm font-bold border transition-colors ${method === m.value ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
                  {m.label}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wide block mb-2">Score — {teamName(match.team1_id)}</label>
              <input type="number" min="0" step="1" value={score1} onChange={e => setScore1(e.target.value)}
                placeholder="0"
                className="w-full border border-gray-200 rounded-lg px-4 py-3 text-base text-xl font-mono focus:outline-none focus:ring-2 focus:ring-purple-300" />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wide block mb-2">Score — {match.team2_id ? teamName(match.team2_id) : 'Team 2'}</label>
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3}
              ...
              className="w-full border border-gray-200 rounded-lg px-4 py-3 text-base text-sm focus:outline-none focus:ring-2 focus:ring-purple-300 resize-none" />
          </div>
