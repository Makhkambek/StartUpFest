                    ...
                    <table className="w-full min-w-[300px] text-sm">
                      ...
                      <tbody className="divide-y divide-gray-50 dark:divide-zinc-800">
                        {(() => {
                          ...
                          return (
                            <tr key={m.id} onClick={() => { if (!done) router.push('/judges/a/record/' + m.id) }} className={`transition-colors ${!done ? 'cursor-pointer' : ''} hover:bg-gray-50 dark:hover:bg-zinc-800 ${isNext ? 'bg-blue-50/60 dark:bg-blue-950/30 border-l-4 border-l-blue-500' : isOpen ? 'bg-amber-50 dark:bg-amber-950/20' : ''}`}>
                              ...
                              <td className="px-2 py-3">
                                <div className="flex items-center justify-end gap-1">
                                  ...
                                  {done && (
                                    <button onClick={e => { e.stopPropagation(); router.push('/judges/a/record/' + m.id) }}
                                  ...
                                  <button onClick={async e => { e.stopPropagation(); if (await confirm(`Delete ${m.match_id}?`)) deleteMatch(m.id) }}
