  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <header className="bg-white border-b border-gray-200 h-14 flex items-center px-3 sm:px-6 gap-4 sticky top-0 z-10">
        <button onClick={() => router.push('/judges/a')} className="text-sm text-gray-400 hover:text-gray-700">← Back</button>
      ...
      <div className="p-3 sm:p-6 max-w-lg mx-auto space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="text-xl font-semibold text-gray-700 mt-1">{teamName(match.team1_id)}</div>
            </div>
            <div className="flex flex-col gap-1.5">
              ...
              {matchStatus === 'completed' && (
                <span className="bg-green-100 text-green-700 text-xs font-black px-3 py-2 rounded-lg text-center">
                  ✓ Completed
                </span>
              )}
            </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-5">
          {/* Phase */}
          <div>
            <label className="text-xs font-bold text-gray-400 uppercase tracking-wide block mb-2">Phase</label>
            <div className="flex gap-2">
              {(['qualification', 'final'] as const).map(p => (
                <button key={p} onClick={() => setPhase(p)}
                  className={`flex-1 py-3 rounded-lg text-sm font-bold border transition-colors ${phase === p ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
          ...
          {/* Penalty */}
          <div>
            <label className="text-xs font-bold text-gray-400 uppercase tracking-wide block mb-2">Penalty</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wide block mb-2">Run 1 (s)</label>
              <input type="number" step="0.01" value={run1} onChange={e => { setRun1(e.target.value); setPen1(0) }}
                placeholder="12.34"
                className="w-full border border-gray-200 rounded-lg px-4 py-3 text-base text-xl font-mono focus:outline-none focus:ring-2 focus:ring-amber-300" />
              <div className="flex flex-wrap gap-1.5 mt-2">
                <button type="button" onClick={() => addPenalty(1, 20)}
                  className="flex-1 py-2 rounded-md text-xs font-bold border border-gray-200 text-gray-700 hover:bg-amber-50 hover:border-amber-300 active:bg-amber-100">
                  +20s
                </button>
                ...
                <button type="button" onClick={() => resetPenalty(1)} disabled={pen1 === 0}
                  className="px-2 py-2 rounded-md text-xs font-bold border border-gray-200 text-gray-500 hover:bg-gray-50 disabled:opacity-30">
                  ↻
                </button>
              </div>
              {pen1 > 0 && (
                <div className="text-[11px] text-amber-700 font-bold mt-1.5">Penalty applied: +{pen1}s</div>
              )}
            </div>
            <div>
              <label className="text-xs font-bold text-gray-400 uppercase tracking-wide block mb-2">Run 2 (s)</label>
              ...
              {pen2 > 0 && (
                <div className="text-[11px] text-amber-700 font-bold mt-1.5">Penalty applied: +{pen2}s</div>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3}
              ...
              className="w-full border border-gray-200 rounded-lg px-4 py-3 text-base text-sm focus:outline-none focus:ring-2 focus:ring-amber-300 resize-none" />
          </div>
          ...
          {savedJustNow ? (
            <div className="bg-green-50 border-2 border-green-200 rounded-xl p-4 space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={undo}
                  className="flex-1 bg-white border-2 border-red-300 text-red-700 py-3 rounded-lg text-sm font-bold hover:bg-red-50">
                  ↩ Undo
                </button>
