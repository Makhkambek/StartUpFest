...
export default function StandingsTableB({ standings }: { standings: StandingB[] }) {
  ...
  const headers = [tc('rank'), tc('team'), tb('group'), tb('wdl'), tb('roundWins'), tc('points'), tc('status')]
  return (
      ...
      <tbody>
        {standings.map((row) => (
          <tr key={row.team.id} className="hover:bg-gray-50 border-b border-gray-100 last:border-0">
            ...
            <td className="px-3 sm:px-6 py-2 sm:py-3 text-gray-700">{tb('groupPrefix', { letter: row.team.group_letter ?? '—' })}</td>
            <td className="px-3 sm:px-6 py-2 sm:py-3">
            ...
            <td className="px-3 sm:px-6 py-2 sm:py-3"><span className={STATUS_CLS[row.status]}>{tc(STATUS_LABEL_KEY[row.status])}</span></td>
          </tr>
        ))}
      </tbody>
    </table>
