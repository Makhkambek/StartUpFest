      ...
      <tbody>
        {standings.map((row) => (
          <tr key={row.team.id} className="hover:bg-gray-50 border-b border-gray-100 last:border-0">
            ...
            <td className="px-6 py-3 text-gray-700">Group {row.team.group_letter}</td>
            <td className="px-6 py-3">
            ...
            <td className="px-6 py-3 text-gray-800">{row.round_wins}</td>
            <td className="px-6 py-3 font-semibold text-gray-900">{row.points}</td>
            <td className="px-6 py-3"><span className={STATUS_CLS[row.status]}>{STATUS_LABEL[row.status]}</span></td>
          </tr>
        ))}
      </tbody>
    </table>
